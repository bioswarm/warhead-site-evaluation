"""
PyMOL 通用共价 warhead 位点筛选脚本 v3.0.1
==========================================
...
7. PyMOL 画面最多标注三个唯一 binder 位点，例如 `#1 A:Ser74->Cys`，
   target 标签例如 `B:Tyr151`；其他候选只打印并写入 CSV。
...
interface_cutoff,candidate_cutoff,ideal_min,ideal_max,output_prefix`。
"""
...
def _is_positive_atom(atom):
    return str(getattr(atom, "name", "")) in POSITIVE_ATOMS.get(
        str(getattr(atom, "resn", "")).upper(), set()
    )


def _is_negative_atom(atom):
...
def _contact_pairs(atoms1, atoms2, cutoff):
    ...
    for atom1 in _heavy(atoms1):
        for atom2 in _heavy(atoms2):
            if _distance(atom1.coord, atom2.coord) <= float(cutoff):
                count += 1
    return count


def _polar_contact_pairs(atoms1, atoms2, cutoff):
    count = 0
    for atom1 in _heavy(atoms1):
        if not _is_polar_atom(atom1):
            continue
        for atom2 in _heavy(atoms2):
            if _is_polar_atom(atom2) and _distance(atom1.coord, atom2.coord) <= float(cutoff):
                count += 1
    return count


def _polar_min_distance(atoms1, atoms2):
    best = None
    for atom1 in _heavy(atoms1):
        if not _is_polar_atom(atom1):
            continue
        for atom2 in _heavy(atoms2):
            if not _is_polar_atom(atom2):
                continue
            value = _distance(atom1.coord, atom2.coord)
...
def _salt_bridge_pairs(atoms1, atoms2, cutoff):
    count = 0
    for atom1 in _heavy(atoms1):
        for atom2 in _heavy(atoms2):
            opposite = (_is_positive_atom(atom1) and _is_negative_atom(atom2)) or (
                _is_negative_atom(atom1) and _is_positive_atom(atom2)
            )
            if opposite and _distance(atom1.coord, atom2.coord) <= float(cutoff):
                count += 1
    return count
...
def _resolve_profile(profile="broad_nucleophile", reactive_atoms="", target_types=None):
    if isinstance(profile, WarheadProfile):
        base = profile
    else:
        name = str(profile or "broad_nucleophile").strip().lower()
        if name == "custom":
            base = WarheadProfile("custom", _parse_reactive_atoms_spec(reactive_atoms), "User-defined profile")
    ...
    requested = _parse_target_types(target_types)
...
def _build_cys_rotamers(res_atoms, resn):
    ...
    if resn == "CYS":
        ...
        if sg_atom is not None and cb_atom is not None:
            return [{
                "chi1_deg": None, "chi1_label": "native", "sg_coord": tuple(sg_atom.coord),
                "cb_coord": tuple(cb_atom.coord), "source": "native_cys",
            }]
...
def _find_disulfide_residues(*group_sets):
    cys = []
...
def _apply_class_caps(uncapped_class, caps):
    ...
    final_class = uncapped_class
    if cap_class and CLASS_ORDER[cap_class] > CLASS_ORDER[final_class]:
        final_class = cap_class
    reasons = [reason for cap, reason in caps if CLASS_ORDER.get(cap, -1) >= CLASS_ORDER.get(final_class, -1)]
    if final_class == uncapped_class:
        reasons = []
    return final_class, cap_class, ";".join(dict.fromkeys(reasons))
...
def _compute_binder_feature(
    ...
    polar_contacts = _polar_contact_pairs(sidechain, all_target_heavy, config.polar_cutoff) if sidechain else 0
    polar_min_distance = _polar_min_distance(sidechain, all_target_heavy) if sidechain else None
    salt_bridges = _salt_bridge_pairs(sidechain, all_target_heavy, config.salt_bridge_cutoff) if sidechain else 0
    ...
    if free_asa is not None and MAX_ASA.get(bresn):
        relative_sasa = max(0.0, float(free_asa) / MAX_ASA[bresn])
    ...
    risk_flags = []
    if bresn == "PRO": risk_flags.append("pro_backbone")
    if bresn == "GLY": risk_flags.append("gly_backbone")
    if bresn in AROMATIC: risk_flags.append("aromatic_hotspot_risk")
    if bresn in CHARGED: risk_flags.append("charged_residue")
    ...
    rotamers = _build_cys_rotamers(batoms, bresn) if config.compute_rotamers else []
    binder_other_atoms = _truncated_environment(binder_res, bkey, include_mutated_backbone=False)
    binder_exposure_atoms = _truncated_environment(binder_res, bkey, include_mutated_backbone=True)
    target_atoms = all_target_heavy
    ...
    anchor_coord, anchor_name = _anchor_coord(batoms, bresn)
    return {
        "binder_key": bkey,
        "binder_chain": bch,
        "binder_resi": bresi,
        ...
        "mutability_score": MUTABILITY.get(bresn, -0.5),
...
def _precompute_binder_features(
    ...
    sasa_free=None, sasa_bound=None, candidate_keys=None,
):
    ...
    for bkey in sorted(keys, key=_residue_sort_key):
        features[bkey] = _compute_binder_feature(
            ...
            sasa_free=sasa_free, sasa_bound=sasa_bound,
        )
    return features
...
def _evaluate_pair_for_target_atom(feature, target_key, target_atom, environment_atoms, config):
    ...
    if fallback_distance is not None and fallback_distance < config.ideal_min:
        risk_flags.append("tighter_than_preferred")
    elif fallback_distance is not None and fallback_distance > config.ideal_max:
...
def _pair_target_label(record):
    return "%s:%s%s(%s)" % (
        record["target_chain"], record["target_resn"],
        record["target_resi"], record["target_atom"],
    )
...
def _analyze_groups(
    ...
    resolved_profile = _resolve_profile(profile, reactive_atoms=reactive_atoms, target_types=target_types)
    config = ScanConfig(
    ...
    disulfide_bound = _find_disulfide_residues(target_res, binder_res)
    target_records = []
    target_context = []
    for tkey in sorted(target_res, key=_residue_sort_key):
        tatoms = target_res[tkey]
        tch, tresi, tresn = tkey
        wanted = resolved_profile.reactive_atoms.get(tresn)
        if not wanted:
            continue
        ...
        if reactive_min > config.interface_cutoff:
            continue
        is_disulfide = tresn == "CYS" and tkey in disulfide_bound
    ...
    if all_reactive_atoms:
        for bkey, batoms in binder_res.items():
            anchor_coord, anchor_name = _anchor_coord(batoms, bkey[2])
            if anchor_coord is None:
                continue
            nearest = min(_distance(anchor_coord, atom.coord) for atom in all_reactive_atoms)
    ...
    environment_atoms = all_target_heavy + all_binder_heavy
    pair_records = []
        ...
        for bkey in sorted(binder_features, key=_residue_sort_key):
            ...
            for target_atom in reactive_atoms_list:
                candidate = _evaluate_pair_for_target_atom(
                    feature, tkey, target_atom, environment_atoms, config
                )
    ...
    for record in binder_records:
        record["target_options"] = ",".join(sorted(
            ...
            key=lambda label: (
                str(label).split(":", 1)[0],
                _natural_residue_key(re.sub(r"\D", "", str(label)) or "0"),
                str(label),
            ),
        ))
...
def _build_scan_selections(object_name, target_chain="", binder_chain="", target_sel="", binder_sel=""):
...
def _resolve_object(object_name="auto"):
    ...
    if value.lower() in {"", "auto", "none"}:
        ...
        objects = cmd.get_object_list("(%s)" % _model_clause(value))
        if not objects and value in cmd.get_names("objects"):
            objects = [value]
    if len(objects) != 1:
        available = cmd.get_object_list("(polymer.protein)")
        raise ValueError("Select exactly one molecular object. Available: %s" % (
...
TARGET_FIELDS = [
    "target_chain", "target_resi", "target_resn", "reactive_atoms",
...
PAIR_FIELDS = [
    ...
    "downgrade_reasons", "binder_chain", "binder_resi", "binder_resn",
    "target_chain", "target_resi", "target_resn", "target_atom", "anchor_atom",
...
def _pretty_resn(resn):
    text = str(resn or "").strip()
    return text[:1].upper() + text[1:].lower() if text else "Unk"


def _mutation_site_label(record):
    return "%s:%s%s->Cys" % (
        record["binder_chain"], _pretty_resn(record["binder_resn"]), record["binder_resi"]
    )


def _target_site_label(record):
    return "%s:%s%s" % (
        record["target_chain"], _pretty_resn(record["target_resn"]), record["target_resi"]
    )
...
def _visualize(object_name, result, label_top=3, top_n=None):
    ...
    for index, binder_record in enumerate(result.get("binder_records", [])[:label_top], 1):
        ...
        if all(value is not None for value in sg_values):
            cmd.pseudoatom(label_name, pos=list(sg_values), label="#%d %s" % (index, _mutation_site_label(binder_record)))
        else:
            anchor_sel = "(%s) and name %s" % (binder_sel, pair.get("anchor_atom") or "CB")
            cmd.pseudoatom(label_name, selection=anchor_sel, label="#%d %s" % (index, _mutation_site_label(binder_record)))
        try:
...
def _print_binder_rank_table(records, top_n=0, label_top=3):
    print("Mark Rank Binder         Final Raw   Uncap Cap Best target          SG-dist  Downgrade reason")
    labelled = max(0, min(3, int(label_top)))
    for record in _records_for_console(records, top_n):
        site = "%s:%s%s" % (record["binder_chain"], record["binder_resn"], record["binder_resi"])
        ...
        print("%-5s %4d %-14s %5s %5.2f %5s %3s %-20s %7s  %s" % (
            mark, int(record["binder_rank"]), site, record["final_class"],
...
def warhead_scan(
    ...
    target_selection, binder_selection = _build_scan_selections(
        ...
        target_sel=target_sel, binder_sel=binder_sel,
    )
    ...
    if not result["target_records"]:
        ...
        for record in result["target_records"]:
            ...
            print("  %s:%s%s  reactive_min=%.2f Å  %s" % (
                record["target_chain"], record["target_resn"], record["target_resi"],
...
def _prompt_objects():
    ...
    objects = list(cmd.get_object_list("(polymer.protein)"))
    if not objects:
...
def _prompt_chains(object_name):
    try:
        chains = cmd.get_chains("(%s) and polymer.protein" % _model_clause(object_name))
    except TypeError:
        chains = cmd.get_chains("(%s) and polymer.protein" % _model_clause(object_name), 1)
    return sorted(set(str(chain) for chain in chains), key=_natural_residue_key)
...
BATCH_FIELDS = [
    ...
    "compute_rotamers", "include_disulfide_targets", "output_prefix",
]
...
def warhead_batch(
    ...
    for index, row in enumerate(rows, 1):
        ...
        record = {
            ...
            "n_binder_sites": 0, "top1_site": "", "top1_class": "",
        ...
        try:
            ...
            if result.get("binder_records"):
                ...
                record.update({
                    "top1_site": "%s:%s%s" % (top["binder_chain"], top["binder_resn"], top["binder_resi"]),
    ...
    fields = [
        ...
        "n_pairs", "n_binder_sites", "top1_site", "top1_class",
        "top1_raw_score", "top1_target",
    ]
